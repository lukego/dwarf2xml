...
<xsl:template match="/dwarf/debug_pubnames">
  <h2><a name ="h2_{local-name()}" />.debug_pubnames - Lookup by name</h2>
  <table border="1">
    <tr >
      <th>name</th>
      <th>compile unit</th>
  ...
  <xsl:for-each select="pubname">
    <tr>
      <td><a href="#{@ref}"><xsl:value-of select="."/></a></td>
      <td><a href="#{@cu}" ><xsl:value-of select="key('dies', @cu)/at_name"/></a></td>
      <td>off <xsl:value-of select="substring-after(@ref,':')"/></td>
    </tr>
    </xsl:for-each>
  </table>
</xsl:template>
